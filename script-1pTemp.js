    ...
    let lastKnownP1Pos = { row: null, col: null };
    let assumedP1Pos = { row: null, col: null };
    let grid = [];
    // Movement offsets
    const directions = {
        w: [-1, 0], // up
    ...
    document.addEventListener("keydown", (e) => {
        ...
        const allowedKeys = ["w", "a", "s", "d", "c", "g", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Enter", "/", "Shift", " "]; // spacebar = " "
        const actionKey = e.key.toLowerCase();

        // Ignore keys not in the allowed list
        if (!allowedKeys.includes(actionKey)) return;

        // Optional: prevent browser actions (like scrolling when pressing space)
        e.preventDefault();
    ...
    function processActionKey(actionKey){
        ...
        if (gameOver) return;
         ...
         turnIndicator.style.color = "lightgray"; // Player 2 cue
         turnIndicator.style.backgroundColor = "blue";
         processPlayerMove(actionKey, "P2");
    ...
    document.getElementById("btn-up").addEventListener("click", (e) => simulateKeyPress("ArrowUp", e.target));
    document.getElementById("btn-down").addEventListener("click", (e) => simulateKeyPress("ArrowDown", e.target));
    document.getElementById("btn-left").addEventListener("click", (e) => simulateKeyPress("ArrowLeft", e.target));
    document.getElementById("btn-right").addEventListener("click", (e) => simulateKeyPress("ArrowRight", e.target));
    ...
    document.getElementById("btn-camouflage").addEventListener("click", (e) => simulateKeyPress("Shift", e.target));
    document.getElementById("btn-glow").addEventListener("click", (e) => simulateKeyPress("Enter", e.target));
    document.getElementById("btn-attack").addEventListener("click", (e) => simulateKeyPress("/", e.target));
